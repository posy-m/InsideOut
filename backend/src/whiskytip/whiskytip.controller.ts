...
export class WhiskytipController {
  ...
  async uploadFile(
    ...
    try {
      ...
      await this.whiskytipService.creatTip(data);
  ...
  async addComment(
    @Res() res: Response,
    @Body('nickname') nicknname: string,
    @Body('id') id: number,
  ...
  // 대댓글 post
  @Post('Ccomment')
  async addCcomment(@Res() res: Response, @Body('nickname') nickname: string, @Body('id') id: number, @Body('CcommentText') Ccomment: string) {
  ...
  // 대댓글 get
  @Get('Ccommentcheck/:id')
  async checkCcomment(@Res() res: Response, @Param('id', ParseIntPipe) id: number) {
    const tips = await this.whiskytipService.checkCcomment(id);
    res.json(tips)
  }
  ...
  async updateCcomment(@Res() res: Response,
    @Body('nick_name') nick_name: string,
    @Param('id', ParseIntPipe) id: number, // 글 id
    @Body('commentId', ParseIntPipe) tip_comment_id: number, // 커멘트아이디
    @Body('comment') tip_com_comment: string,) {
    const tips = await this.whiskytipService.updatdCcomment(tip_comment_id, id, nick_name, tip_com_comment)
    res.json(tips);
  }
  ...
  async deleteCcomment(@Res() res: Response, @Param('id', ParseIntPipe) id: number, // 글 id
    @Body('nick_name') nick_name: string, // 닉네임
    @Body('comment_id', ParseIntPipe) tip_comment_id: number, // 글 id)
  ) {
    const tips = await this.whiskytipService.deleteCcomment(
      id,
      nick_name,
      tip_comment_id
    )
