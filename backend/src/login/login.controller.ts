...
export class LoginController {
  ...
  async kakaoAuthLogin(@Query('code') code : string){
    return await this.loginService.valiDateKakao(code);
  }

  @Get('google')
  googleLogin(@Res() res : Response){
    const MY_CLIENT_ID = process.env.GOOGLE_CLIENT_KEY;
    const MY_REDIRECT_URI = process.env.GOOGLE_SECRET_KEY;
  ...
  async googleAuthLogin(@Query('code') code : string){
    return await this.loginService.valiDateGoogle(code);
  }
}
